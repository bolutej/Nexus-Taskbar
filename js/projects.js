...
sortMenu.className = 'hidden absolute right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-50 py-1 min-w-[160px]';
sortMenu.innerHTML = Object.entries(sortOptions).map(([key, val]) => `
  <button
    class="sort-option w-full text-left px-4 py-2 text-sm text-[#172b4d] hover:bg-slate-50 transition-colors"
    data-sort="${key}">
    ${val.label}
  </button>
`).join('');
...
sortBtn.parentElement.appendChild(sortMenu);
...
sortMenu.addEventListener('click', async (e) => {
  ...
  sortBtn.innerHTML = `
    <svg class="text-[#5e6c84]" viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3 5h18l-7 9v6l-4-2v-4L3 5z" />
    </svg>
    ${sortOptions[currentSort].label}
  `
  await loadProjects(currentSort);
});
